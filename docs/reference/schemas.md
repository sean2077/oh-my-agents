...
- `participant_sessions` claims each author slot for one concrete session hash. The creator is claimed at pair creation; the peer is claimed by `pair join` or single-active auto-adopt. Same-author different-session reuse is refused fail-closed.
