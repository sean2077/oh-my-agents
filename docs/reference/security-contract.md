...
- `--dry-run` is a global persistent flag, inherited by all mutating commands (the full asset family, `state set`, `relay draft`/`publish`/`close`/`init`, `self-update`): it runs the complete compute-and-validate path, prints the **exact absolute paths** to be created/modified/deleted along with the operation type, and guarantees zero persistent target writes (no canonical asset changes, backups, registry/state/ledger mutation, or host config changes). A command that must validate a remote release artifact may use an auto-cleaned private temp directory outside the target state; leaving temp residue is a failure.
- Test: after a dry-run, snapshot-compare the target directory tree and assert no persistent change; remote-validation tests also assert temp cleanup on failure.
