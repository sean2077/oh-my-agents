...
Autopilot state uses the CLI's global workflow session scope. For a normal run,
call `oma --session current state ...` with the logical namespace `autopilot`;
the CLI stores it as `autopilot-<session>` in the shared project `.oma/state/`.
If `current` cannot resolve a platform session, set `OMA_SESSION_ID=<slug>` or
pass an explicit `--session <slug>`. The legacy unscoped namespace `autopilot`
is reserved only for resuming an older single-run state file.

```
oma --session current state set autopilot/phase <clarify|plan|implement|verify|deliver|done>
oma --session current state set autopilot/goal "<one-line goal>"
oma --session current state set autopilot/plan-path <path>
```
...
```
oma --session current state get autopilot/phase
```

- **Missing key (exit 3) or `done`** → no active run in that session. For a new authorized task, initialize before any work: `oma --session current state set autopilot/goal "<one-line goal>"`, then `oma --session current state set autopilot/phase clarify`.
...
```
oma --session current state list autopilot --json
```
...
scope, ask which namespace to resume rather than guessing. Without
`--session`, `oma state list autopilot --json` shows project-wide autopilot
...
5. **deliver** — summarize what changed, the verification evidence, and anything deferred; set `autopilot/phase` to `done` with `oma --session current state set`. Delivery is a report to the user, not a merge/push decision — those remain theirs unless pre-authorized. For cross-reviewed delivery (a high-impact change, or when the user wants a second agent's sign-off), hand off to the `pair-delivery` skill as a bounded subflow instead of self-approving here — autopilot composes that primitive rather than reinventing review.
...
1. Phase state lives in `oma --session current state` — if you did work without updating the phase, fix the state before continuing.
