...
- **`--session <slug|current>`**: a **global workflow-state scope**, inherited by `state`, `interview`, and `ralph`. `current` resolves `CODEX_THREAD_ID`, `CLAUDE_CODE_SESSION_ID`, or `OMA_SESSION_ID` into a path-safe suffix. Without `--session`, commands preserve the legacy project-global behavior.
