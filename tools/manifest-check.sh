...
#
# Usage:
#   bash tools/manifest-check.sh [path/to/manifest.tsv]
#                                                    # default: tools/tools-manifest.tsv
# Env:
...
MANIFEST="${1:-$ROOT/tools/tools-manifest.tsv}"
case "$MANIFEST" in
    /* | [A-Za-z]:/*) ;;
    *) MANIFEST="$ROOT/$MANIFEST" ;;
esac
[[ -f "$MANIFEST" ]] || { echo "manifest not found: $MANIFEST" >&2; exit 2; }
SCAN_DIR="${TOOLS_DIR:-$ROOT}"
[[ -d "$SCAN_DIR" ]] || { echo "scan directory not found: $SCAN_DIR" >&2; exit 2; }
SKIP_RE="${MANIFEST_CHECK_SKIP:-(^|/)(\.agents|assets|eval|internal|vendor|tests?|legacy|\.git|\.worktrees)/|(^|/)[._]}"
