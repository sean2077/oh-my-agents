...
3. **资产模型** `{skill, subagent, hook, command/prompt}` 经 `~/.agents/{skills,agents,hooks}/` 规范位**软链**投影到 **Claude Code 与 Codex** 两端（hook 为 canonical-only，oma 不改宿主配置——见 §安装规范的 2026-06-15 修订）；
...
- **安装规范**：资产文件落 `~/.agents/{skills,agents,hooks}/` 规范位；`~/.claude/skills` 等 agent 目录放软链。**修订（2026-06-15）**：hooks 不再以 fragment 注入双端宿主配置——hook 资产改为 canonical-only（仅规范位放置 `fragment.json`），用户照其内容手动接入自己的 `settings.json`/`hooks.json`（oma 零宿主配置改写；接线规范见 relay-v2-protocol.md §12.4）。
...
- oma 零宿主配置改写：hook 资产为 canonical-only（不注入 `settings.json`/`hooks.json`，用户手动接线）——2026-06-15 起，原 hook fragment 注入机制已移除
...
- [ ] 安全与权限要求落地：--dry-run、备份/--force 覆盖策略、软链可信根约束、限制性权限、oma 零宿主配置改写（hook canonical-only）
...
| Hook 片段 | supporting | canonical-only 放置；用户手动接线（2026-06-15 起不再注入宿主） | 属于内容资产 |
