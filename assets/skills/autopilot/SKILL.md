...
5. **deliver** — summarize what changed, the verification evidence, and anything deferred; set `autopilot/phase done`. Delivery is a report to the user, not a merge/push decision — those remain theirs unless pre-authorized. For cross-reviewed delivery (a high-impact change, or when the user wants a second agent's sign-off), hand off to the `pair-delivery` skill as a bounded subflow instead of self-approving here — autopilot composes that primitive rather than reinventing review.
