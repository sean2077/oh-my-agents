...
> **`/goal` driver (optional, host-native)**: when `verify` runs the ralph subflow and the current interactive surface exposes `/goal`, it may auto-continue those rounds — see ralph's capability gate and fallback. The phase contract is unchanged: oma still judges stop, and `autopilot/phase` advances past `verify` only on ralph `passed`.
