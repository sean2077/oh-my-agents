...
This IS the "research profile" — a ralph preset expressed through `--keep-policy score_improvement`, not a separate command. The default current workflow session is the host-session boundary inside the shared project `.oma/state/`; `--id` names the mission within that session. `--max-rounds` bounds the search (the `exhausted` terminal below); set it deliberately, since the default of 10 is often too few for a real optimization run.
