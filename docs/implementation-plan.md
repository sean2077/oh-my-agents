...
7. **CI**：golangci-lint + 全部门禁 + release 构建脚本（jira-cli 模式：scripts/build-release.sh 交叉编译 6 平台 + checksums.txt + tag 版本门禁；用户指定参考 jira-cli，2026-06-11 已落 .github/workflows/{ci,release}.yml）。**发布流程（用户定义 2026-06-11）**：用户决定发布 → 更新 CHANGELOG.md（面向 release 页面）→ commit + tag → push 触发 CI → `scripts/extract-changelog.sh` 截取 tag 对应版本节作 Release 正文；tag 无对应节 → 发布 fail-closed（先于构建快速失败）
...
| B11 | self-update（checksum 校验、限定更新源、原子替换+回退、--check 只读）；release 构建链（脚本 + tag 版本门禁）已于 2026-06-11 前移落地 | 安全测试：checksum 不匹配/元数据不可用/不可写/中断/回退 |
