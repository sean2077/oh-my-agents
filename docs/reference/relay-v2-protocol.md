...
**Switch gate**: agent-ledger relay is decommissioned only once the auto-continue-turn dispatcher, verified through manual wiring, is at parity or better. The public relay group is exactly 8 in its terminal shape (init/pair/draft/publish/wait/status/close/preflight; the `hook <event>` dispatcher is hidden and not counted). The compact status line is the top-level `oma statusline` (command-tree.md §7) — it supersedes the former relay `statusline` and renders whichever core workflow the session is in.
